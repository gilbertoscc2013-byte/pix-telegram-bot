...
async def pix(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        # E-mail usado pelo Mercado Pago para identificar o pagador
        email = f"telegram_{update.effective_user.id}@example.com"
        ...
        # Chave única para evitar criação duplicada da cobrança
        ...
        if status not in ["pending", "in_process"]:
            await update.message.reply_text(
                f"❌ O Mercado Pago retornou um erro.\n"
                f"Status: {status}"
            )
            print("Resposta Mercado Pago:", payment)
            return
        ...
        if qr_code:
            mensagem += (
                "📋 *Pix Copia e Cola:*\n\n"
                f"`{qr_code}`\n\n"
                "Copie o código acima e faça o pagamento pelo seu banco."
            )
        else:
            mensagem += (
                "⚠️ O Mercado Pago não retornou o código Pix.\n"
        ...
        print("ERRO AO CRIAR PIX:", repr(e))
